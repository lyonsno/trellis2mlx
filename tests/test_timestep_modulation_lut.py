...
SPARSE_TIMESTEP_BITS = np.asarray(
    [
        0x447A0000,
        0x44730E39,
        0x446A6000,
...
SHAPE_SCHEDULE_PROFILE = "shape-slat-rescale-3"
SPARSE_SCHEDULE_PROFILE = "sparse-structure-rescale-5"
...
def _write_source_pair(
    ...
    *,
    ...
    schedule_profile=SHAPE_SCHEDULE_PROFILE,
...
def test_source_cuda_timestep_modulation_lut_binds_sparse_schedule_profile(
    tmp_path,
):
    ...
    npz_path, report_path, npz_sha256, report_sha256 = _write_source_pair(
        tmp_path,
        schedule_profile=SPARSE_SCHEDULE_PROFILE,
        timestep_bits=SPARSE_TIMESTEP_BITS,
    )
    lut = load_source_cuda_timestep_modulation_lut(
        ...
        expected_schedule_profile=SPARSE_SCHEDULE_PROFILE,
    )

    assert lut.report_identity()["schedule_profile"] == SPARSE_SCHEDULE_PROFILE
    with pytest.raises(ValueError, match="schedule profile"):
        load_source_cuda_timestep_modulation_lut(
            npz_path=npz_path,
            report_path=report_path,
            expected_npz_sha256=npz_sha256,
            expected_report_sha256=report_sha256,
            expected_source_checkpoint_sha256=SOURCE_CHECKPOINT_SHA256,
            expected_schedule_profile=SHAPE_SCHEDULE_PROFILE,
        )
...
def test_sparse_shared_modulation_uses_lut_without_falling_back(tmp_path):
    import mlx.core as mx

    from trellmlx.models.sparse_structure_flow import _sparse_shared_modulation
    from trellmlx.timestep_modulation_lut import (
        load_source_cuda_timestep_modulation_lut,
    )
    ...
    timestep = mx.array(
        [float(SPARSE_TIMESTEP_BITS.view(np.float32)[5])],
        dtype=mx.float32,
    )

    modulation = _sparse_shared_modulation(
        timestep,
        t_embedder=None,
        adaLN_modulation=None,
        compute_dtype=mx.bfloat16,
        sparse_timestep_modulation_lut=lut,
    )
    mx.eval(modulation)
    modulation_bits = (
        np.asarray(modulation.astype(mx.float32))
...
def test_stage_capture_threads_authenticated_sparse_timestep_modulation_route(
    tmp_path,
):
    from scripts.run_mlx_stage_capture import (
        _build_generate_command,
    ...
    for flag, expected in (
        ("--sparse-timestep-modulation-lut", str(npz_path)),
        ("--sparse-timestep-modulation-report", str(report_path)),
        ("--expected-sparse-timestep-modulation-lut-sha256", npz_sha256),
        ("--expected-sparse-timestep-modulation-report-sha256", report_sha256),
        (
            "--expected-sparse-timestep-modulation-source-checkpoint-sha256",
            SOURCE_CHECKPOINT_SHA256,
        ),
        ...
        assert command[command.index(flag) + 1] == expected
    assert route["sparse_timestep_modulation_lut_path"] == str(npz_path)
    assert route["sparse_timestep_modulation_lut_sha256_effective"] == npz_sha256
    assert route["sparse_timestep_modulation_report_path"] == str(report_path)
    assert (
        route["sparse_timestep_modulation_report_sha256_effective"]
        == report_sha256
    )


def test_stage_capture_binds_effective_sparse_timestep_modulation_identity(
    tmp_path,
):
    from scripts.run_mlx_stage_capture import (
    ...
    identity = {
        ...
        "step_indices": list(range(8)),
        "timestep_float32_bits": [
            f"0x{int(value):08x}" for value in SPARSE_TIMESTEP_BITS
        ],
        "modulation_shape": [8, 9216],
    }
    route_identity = {
        "route": {"sparse_timestep_modulation_identity": identity}
    }
    ...
    binding = _bind_effective_sparse_timestep_modulation_identity(
        route_identity,
        checkpoint,
    )

    assert binding["sparse_timestep_modulation_route"] == identity
    assert binding["sha256"] == _sha256(checkpoint)
    assert (
        route_identity["route"][
            "sparse_timestep_modulation_identity_effective"
        ]
        == identity
    )
...
@pytest.mark.parametrize("checkpoint_value", [None, "", '{"counterfeit":true}'])
def test_stage_capture_rejects_missing_or_substituted_effective_sparse_modulation(
    tmp_path,
    checkpoint_value,
):
    from scripts.run_mlx_stage_capture import (
    ...
    payload = {}
    if checkpoint_value is not None:
        payload["sparse_timestep_modulation_lut_json"] = np.asarray(
            checkpoint_value
        )
    np.savez(checkpoint, **payload)

    with pytest.raises(ValueError, match="timestep modulation identity"):
        _bind_effective_sparse_timestep_modulation_identity(
            route_identity,
            checkpoint,
        )
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            "requires a sparse-flow or downstream consumer",
        ),
        (
            [
                "--stop-after-stage",
...
def test_stage_capture_rejects_invalid_sparse_timestep_modulation_route(
    tmp_path,
    argv_tail,
    message,
):
    from scripts.run_mlx_stage_capture import (
        _validate_sparse_timestep_modulation_route_args,
        build_parser,
    )
    ...
    if "only.npz" in argv_tail:
        route_args = argv_tail
    else:
        route_args = argv_tail + complete_args
    args = build_parser().parse_args(
        [
            "--image",
            "input.png",
