...
This wrapper is a Greenroom-friendly evidence route for comparing MLX stage
artifacts against the local Trellis-Mac reference route. It writes route
identity before invoking ``generate.py`` and writes a report even when the child
process fails before producing the requested checkpoint.
"""
...
def main(argv: list[str] | None = None) -> int:
    ...
    _write_json(
        output_dir / "run_report.json",
        {
            ...
            "status": "done",
            "route_identity": route_identity,
            "last_trustworthy_phase": f"{args.stop_after_stage}_saved",
            "primary_output_status": "written" if output_written else "missing",
            "failure_phase": None if output_written else "missing_primary_output",
            "exit_code": result.returncode,
            "elapsed_seconds": {"generate_subprocess": elapsed},
            "artifacts": _artifact_status(checkpoint_dir, args.stop_after_stage),
        },
