#!/usr/bin/env python3
"""CUDA witness for TRELLIS sparse-flow cross-attention raw and projection."""
...
SCHEMA = "trellis2mlx.cuda_sparse_cross_attention.v1"
...
def _input_metrics(data: Any) -> dict[str, Any]:
    return {
        ...
        for name in ("cross_q_post_norm", "cross_attention_raw", "cross_attn")
    }
...
def _run(args: argparse.Namespace) -> tuple[dict[str, Any], dict[str, np.ndarray]]:
    ...
    with np.load(args.witness, allow_pickle=False) as data:
        ...
        k = _require(data, "cross_k")
        v = _require(data, "cross_v")
        ...
        for prefix in ("source", "captured"):
            raw, projected = _attention_and_projection(
                q_np=_require(data, f"{prefix}_cross_q_post_norm"),
                k_np=k,
                v_np=v,
                weight_np=weight,
                bias_np=bias,
            ...
            outputs[f"cuda_{prefix}_cross_attention_raw"] = raw
            outputs[f"cuda_{prefix}_cross_attn"] = projected
        ...
        report = {
            ...
            "input_shapes": {
                "q": list(_require(data, "source_cross_q_post_norm").shape),
                "k": list(k.shape),
                "v": list(v.shape),
            ...
            "cuda_source_vs_source_raw": metric_np(
                _require(data, "source_cross_attention_raw"),
                outputs["cuda_source_cross_attention_raw"],
            ),
            "cuda_source_vs_source_cross_attn": metric_np(
                _require(data, "source_cross_attn"),
                outputs["cuda_source_cross_attn"],
            ),
            "cuda_source_vs_captured_raw": metric_np(
                _require(data, "captured_cross_attention_raw"),
                outputs["cuda_source_cross_attention_raw"],
            ),
            "cuda_source_vs_captured_cross_attn": metric_np(
                _require(data, "captured_cross_attn"),
                outputs["cuda_source_cross_attn"],
            ),
            "cuda_captured_vs_captured_raw": metric_np(
                _require(data, "captured_cross_attention_raw"),
                outputs["cuda_captured_cross_attention_raw"],
            ),
            "cuda_captured_vs_captured_cross_attn": metric_np(
                _require(data, "captured_cross_attn"),
                outputs["cuda_captured_cross_attn"],
            ),
            "cuda_captured_vs_source_raw": metric_np(
                _require(data, "source_cross_attention_raw"),
                outputs["cuda_captured_cross_attention_raw"],
            ),
            "cuda_captured_vs_source_cross_attn": metric_np(
                _require(data, "source_cross_attn"),
                outputs["cuda_captured_cross_attn"],
            ),
            ...
            "cuda_source_vs_cuda_captured_cross_attn": metric_np(
                outputs["cuda_source_cross_attn"],
                outputs["cuda_captured_cross_attn"],
            ),
        }
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--witness", type=Path, default=Path("source_sparse_cross_attention_witness.npz"))
...
def main() -> int:
    ...
    try:
        ...
        report = {
            "schema": SCHEMA,
            "status": "failed",
            "failure_phase": "cuda_sparse_cross_attention",
            "witness_path": str(args.witness),
