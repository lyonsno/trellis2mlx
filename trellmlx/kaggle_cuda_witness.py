...
PACKET_STATE_NAME = "packet-state.json"
...
def prepare_packet(packet: KaggleCudaWitnessPacket) -> KaggleCudaWitnessPacket:
    ...
    if _paths_overlap(packet.output_dir, packet.capsule_dir):
        report_path = _packet_path_failure_report(packet.output_dir)
        _write_json_safely(
            report_path,
            {
                "schema": "trellis2mlx.kaggle_cuda_witness.path_validation.v1",
                "status": "failed",
                "failure_phase": "packet_path_validation",
                "error": "packet output directory overlaps the protected capsule",
                "capsule_dir": str(packet.capsule_dir),
                "output_dir": str(packet.output_dir),
            },
        )
        raise WitnessPacketError(
            f"packet output directory overlaps protected capsule: {report_path}"
        )
    ...
    _write_json(
        packet.output_dir / PACKET_STATE_NAME,
        {
            "schema": "trellis2mlx.kaggle_cuda_witness.packet_state.v1",
            "capsule_dir": str(packet.capsule_dir.resolve(strict=False)),
            "dataset_id": packet.dataset_id,
            "kernel_id": packet.kernel_id,
...
def prepare_kernel_output_paths(
    ...
    *,
    ...
    protected_dirs = (packet.capsule_dir, packet.dataset_dir, packet.kernel_dir)
    unsafe = (
        _same_path(output_dir, packet.output_dir)
        or packet.output_dir.resolve(strict=False).is_relative_to(
            output_dir.resolve(strict=False)
        )
        or any(_paths_overlap(output_dir, path) for path in protected_dirs)
        or any(_paths_overlap(report_dir, path) for path in protected_dirs)
        or _paths_overlap(output_dir, report_dir)
    )
    if unsafe:
        failure_path = packet.output_dir / "reports" / "kernel_output_path_validation.json"
        _write_json_safely(
            failure_path,
            {
                "schema": "trellis2mlx.kaggle_cuda_witness.path_validation.v1",
                "status": "failed",
                "failure_phase": "kernel_output_path_validation",
                "error": "kernel output or report directory overlaps protected packet state",
        ...
        raise WitnessPacketError(
            f"kernel output path overlaps protected packet state: {failure_path}"
        )
    return output_dir, report_dir / "kernel_output.json"


def load_prepared_packet(
    output_dir: Path,
    *,
    expected_capsule_dir: Path,
    failure_report_dir: Path,
) -> KaggleCudaWitnessPacket:
    output_dir = Path(output_dir)
    expected_capsule_dir = Path(expected_capsule_dir).resolve(strict=False)
    failure_report_dir = Path(failure_report_dir).resolve(strict=False)
    manifest_path = output_dir / "dataset" / "witness-manifest.json"
    kernel_metadata_path = output_dir / "kernel" / "kernel-metadata.json"
    packet_state_path = output_dir / PACKET_STATE_NAME
    ...
    if not packet_state_path.is_file():
        raise WitnessPacketError(f"missing local packet state: {packet_state_path}")
    manifest = json.loads(manifest_path.read_text())
    kernel_metadata = json.loads(kernel_metadata_path.read_text())
    packet_state = json.loads(packet_state_path.read_text())
    ...
    for field, expected in expected_packet_state.items():
        if packet_state.get(field) != expected:
            raise WitnessPacketError(
                f"local packet state {field} mismatch: expected {expected!r}, "
                f"got {packet_state.get(field)!r}"
            )
    capsule_dir = packet_state.get("capsule_dir")
    claimed_capsule_dir = (
        Path(capsule_dir).resolve(strict=False)
        if isinstance(capsule_dir, str) and Path(capsule_dir).is_absolute()
        else None
    )
    claimed_capsule_valid = (
        claimed_capsule_dir is not None
        and claimed_capsule_dir != Path(claimed_capsule_dir.anchor)
        and claimed_capsule_dir.is_dir()
        and not _paths_overlap(claimed_capsule_dir, output_dir)
    )
    if claimed_capsule_dir != expected_capsule_dir:
        failure_path, failure_temporary = _packet_capsule_identity_failure_paths(
            output_dir,
            failure_report_dir=failure_report_dir,
            expected_capsule_dir=expected_capsule_dir,
            claimed_capsule_dir=claimed_capsule_dir,
            claimed_capsule_valid=claimed_capsule_valid,
        )
        _write_json_safely(
            failure_path,
            {
                "schema": "trellis2mlx.kaggle_cuda_witness.packet_capsule_identity.v1",
                "status": "failed",
                "failure_phase": "packet_capsule_identity_validation",
                "error": "persisted capsule identity does not match caller expectation",
                "packet_dir": str(output_dir.resolve(strict=False)),
                "expected_capsule_dir": str(expected_capsule_dir),
                "claimed_capsule_dir": (
                    str(claimed_capsule_dir) if claimed_capsule_dir is not None else None
                ),
                "claimed_capsule_valid": claimed_capsule_valid,
                "effective_report_path": str(failure_path),
                "effective_report_temporary": str(failure_temporary),
            },
            temporary=failure_temporary,
        )
        raise WitnessPacketError(
            "local packet state capsule identity mismatch: "
            f"expected {expected_capsule_dir}, got {capsule_dir!r}; "
    ...
    packet = KaggleCudaWitnessPacket(
        capsule_dir=expected_capsule_dir,
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    for name in ("dataset-create", "dataset-version", "kernel-push", "kernel-status", "kernel-output", "print-commands"):
        ...
        drive.add_argument("--capsule-dir", type=Path, required=True)
        drive.add_argument("--report-dir", type=Path, required=True)
        if name == "kernel-push":
    ...
    if args.command == "print-commands":
        packet = load_prepared_packet(
            args.packet_dir,
            expected_capsule_dir=args.capsule_dir,
            failure_report_dir=args.report_dir,
        )
        print(json.dumps(_prepared_summary(packet), indent=2, sort_keys=True))
        return 0
    ...
    if args.command == "kernel-output":
        ...
        report_dir = args.report_dir or Path(args.packet_dir) / "reports"
        output_dir, report_path = prepare_kernel_output_paths(
            packet,
            output_dir=output_dir,
...
def _packet_path_failure_report(output_dir: Path) -> Path:
    output_dir = Path(output_dir)
    if output_dir.name:
        return output_dir.with_name(output_dir.name + ".failure.json")
    return Path.cwd() / "kaggle-cuda-witness-packet-path.failure.json"


def _packet_capsule_identity_failure_paths(
    output_dir: Path,
    *,
    failure_report_dir: Path,
    expected_capsule_dir: Path,
    claimed_capsule_dir: Path | None,
    claimed_capsule_valid: bool,
) -> tuple[Path, Path]:
    protected = [Path(expected_capsule_dir)]
    if (
        claimed_capsule_dir is not None
        and claimed_capsule_dir != Path(claimed_capsule_dir.anchor)
        and not _paths_overlap(claimed_capsule_dir, output_dir)
    ):
        protected.append(Path(claimed_capsule_dir))
    roots = (Path(failure_report_dir), Path(output_dir) / "reports")
    for root in roots:
        candidate = root / "packet_capsule_identity_validation.json"
        temporary = candidate.with_name(candidate.name + ".tmp")
        if not any(
            _paths_overlap(mutation, path)
            for mutation in (candidate, temporary)
            for path in protected
        ):
            return candidate, temporary
    raise WitnessPacketError(
        "capsule identity failure report roots overlap protected state: "
...
def _write_json_safely(
    ...
    *,
    ...
    temporary = (
        ...
        else path.with_name(path.name + ".tmp")
    )
    if temporary.parent != path.parent:
        raise WitnessPacketError(
            "atomic JSON temporary must share the report directory: "
            f"{temporary} versus {path}"
        )
