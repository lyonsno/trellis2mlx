...
def unify_face_orientations(faces: np.ndarray) -> np.ndarray:
    """Unify triangle winding so all connected faces have consistent normals.

    BFS across shared edges: if two faces share an edge with the same vertex
    order (both have edge A→B), one must be flipped so they wind consistently.

    Args:
        faces: [F, 3] int64 triangle indices

    Returns:
        faces with consistent winding (may flip some triangles in-place)
    """
    ...
    # For each directed half-edge (a, b) in face f, store (f, local_edge_idx).
    # An edge shared by two faces will appear as (a,b) in one and (b,a) in the
    # other if they're consistently wound, or (a,b) in both if inconsistent.
    edge_to_face = {}
    for fi in range(F):
        for ei in range(3):
            a, b = int(faces[fi, ei]), int(faces[fi, (ei + 1) % 3])
            key = (min(a, b), max(a, b))
            if key in edge_to_face:
                edge_to_face[key].append((fi, a, b))
            else:
                edge_to_face[key] = [(fi, a, b)]

    # BFS: propagate winding from seed faces
    visited = np.zeros(F, dtype=np.bool_)
    flipped = 0
    ...
    for seed in range(F):
        ...
        while queue:
            ...
            for ei in range(3):
                ...
                for (fj, aj, bj) in edge_to_face[key]:
                    if fj == fi or visited[fj]:
                        continue
                    visited[fj] = True

                    # Consistent winding: the shared edge should appear in
                    # opposite directions in adjacent faces.
                    # fi has edge a→b. If fj also has a→b (same direction),
                    # fj needs to be flipped.
                    if a == aj and b == bj:
                        # Same direction — flip fj
                        faces[fj] = faces[fj, ::-1]
                        flipped += 1
