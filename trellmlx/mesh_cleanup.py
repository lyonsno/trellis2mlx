...
def repair_non_manifold_edges(
    ...
    for gi in np.where(manifold_mask)[0]:
        ...
        union(
            face_a * 3 + int(sorted_first_corner[start]),
            face_b * 3 + int(sorted_first_corner[start + 1]),
        )
        union(
            face_a * 3 + int(sorted_second_corner[start]),
            face_b * 3 + int(sorted_second_corner[start + 1]),
        )

    roots = np.array([find(i) for i in range(n_faces * 3)], dtype=np.int64)
    unique_roots, inverse = np.unique(roots, return_inverse=True)
    corner_vertices = faces.reshape(-1)
    new_vertices = vertices[corner_vertices[unique_roots]]
    ...
    if verbose:
        print(
            f"  Split {split_vertices} vertices across "
